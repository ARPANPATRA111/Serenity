...
1. Create or select a staging Vercel project/environment.
2. Configure the staging branch or preview deployment target.
...
5. Confirm Firebase variables point to the staging Firebase project.
6. Confirm email variables are test-mode/safe-inbox only.
7. Deploy only to staging or preview.
...
2. Enable the Auth providers needed by the app, such as email/password and any social providers intentionally used in staging.
...
9. Do not point the staging app at production Firebase.
10. Do not run migrations.
